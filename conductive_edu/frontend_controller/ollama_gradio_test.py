...
def main():
    ...
    with gr.Blocks(title="Ollama 聊天助手") as demo:
        ...
        # 绑定提交事件
        msg.submit(handle_submit, inputs=[msg, chatbot, model_dropdown], outputs=[chatbot, msg])

        submit_btn.click(handle_submit, inputs=[msg, chatbot, model_dropdown], outputs=[chatbot, msg])

        # 模型切换
        model_dropdown.change(lambda m: f"已切换到模型: {m}", inputs=[model_dropdown], outputs=[status_display])
